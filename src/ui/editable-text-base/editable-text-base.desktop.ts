import { autocapitalizationTypeProperty, autocorrectProperty, EditableTextBase as EditableTextBaseCommon, FormattedString, keyboardTypeProperty, returnKeyTypeProperty } from "@nativescript/core/ui/editable-text-base/editable-text-base-common";
...
export abstract class EditableTextBase extends EditableTextBaseCommon {
  ...
  public dismissSoftInput() {
    super.nativeTextViewProtected.resignFirstResponder();
    super.notify({ eventName: EditableTextBaseCommon.blurEvent, object: this });
  }
  ...
  [keyboardTypeProperty.getDefault]():
    ...
    | "email"
    | string {
    let keyboardType = super.nativeTextViewProtected.keyboardType;
    switch (keyboardType) {
  ...
  [keyboardTypeProperty.setNative](
    value: "datetime" | "phone" | "number" | "url" | "email" | string
  ) {
  ...
  [returnKeyTypeProperty.getDefault]():
    ...
    | "send"
    | string {
    let returnKeyType = super.nativeTextViewProtected.returnKeyType;
  ...
  [returnKeyTypeProperty.setNative](
    value: "done" | "next" | "go" | "search" | "send" | string
  ) {
    let newValue;
    switch (value) {
      case "done":
    ...
    super.nativeTextViewProtected.returnKeyType = newValue;
  }
  ...
  [autocapitalizationTypeProperty.getDefault]():
    ...
    | "sentences"
    | "allcharacters" {
    let autocapitalizationType = super.nativeTextViewProtected
    ...
    switch (autocapitalizationType) {
      ...
      default:
        throw new Error(
          "Invalid autocapitalizationType value:" + autocapitalizationType
        );
    }
  }
  [autocapitalizationTypeProperty.setNative](
    value: "none" | "words" | "sentences" | "allcharacters"
  ) {
  ...
  [autocorrectProperty.getDefault](): boolean | number {
    let autocorrectionType = super.nativeTextViewProtected.autocorrectionType;
    switch (
      autocorrectionType
...
export function _updateCharactersInRangeReplacementString(
  ...
  for (let i = 0, length = formattedText.spans.length; i < length; i++) {
    ...
    if (
      currentLocation <= rangeLocation &&
      rangeLocation < currentLocation + span.text.length
    ) {
      let newText = splice(
        span.text,
        rangeLocation - currentLocation,
        deletingText ? rangeLength : 0,
        replacementString
      );
