...
export class GesturesObserver extends GesturesObserverBase {
  ...
  private _attach(target: View, type: GestureTypes) {
    ...
    if (
      target &&
      target.nativeViewProtected &&
      target.nativeViewProtected.addGestureRecognizer
    ) {
      const nativeView = target.nativeViewProtected;

      if (type & GestureTypes.tap) {
        nativeView.addGestureRecognizer(
          this._createRecognizer(GestureTypes.tap)
        );
      }

      if (type & GestureTypes.doubleTap) {
        nativeView.addGestureRecognizer(
          this._createRecognizer(GestureTypes.doubleTap)
        );
      }
        ...
        nativeView.addGestureRecognizer(
          this._createRecognizer(GestureTypes.pan, args => {
            this._executeCallback(
              _getPanData(args, target.nativeViewProtected)
            );
          })
        );
      }
      ...
      if (type & GestureTypes.longPress) {
        nativeView.addGestureRecognizer(
          this._createRecognizer(GestureTypes.longPress)
        );
      }

      if (type & GestureTypes.touch) {
        nativeView.addGestureRecognizer(
          this._createRecognizer(GestureTypes.touch)
        );
      }
    }
  }
    ...
    if (super.target && super.target.nativeViewProtected) {
      for (let name in this._recognizers) {
        if (this._recognizers.hasOwnProperty(name)) {
          ...
          super.target.nativeViewProtected.removeGestureRecognizer(
            item.recognizer
          );
...
class TouchGestureRecognizer {
  ...
  private executeCallback(action: string, touches: Set<any>, event: any): void {
    ...
    this._eventData.prepare(
      (<GesturesObserverBase>this.observer).target,
      action,
      touches,
      event
    );
...
class Pointer implements Pointer {
  ...
  private get location(): any {
    if (!this._location) {
      this._location = this.desktop.locationInView(
        this._view.nativeViewProtected
      );
    }
