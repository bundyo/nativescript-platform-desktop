...
import { categories as traceCategories, isEnabled as traceEnabled, messageType as traceMessageType, write as traceWrite } from "@nativescript/core/trace";
...
function loadFontFromFile(fontFamily: string): any {
  ...
  if (result === undefined) {
    ...
    if (fs.File.exists(basePath + ".ttf")) {
      ...
      if (traceEnabled()) {
        traceWrite(
          "Could not find font file for " + fontFamily,
    ...
    if (fontAssetPath) {
      try {
        ...
        if (traceEnabled()) {
          traceWrite(
            "Error loading font asset: " + fontAssetPath,
            traceCategories.Error,
            traceMessageType.error
          );
        }
