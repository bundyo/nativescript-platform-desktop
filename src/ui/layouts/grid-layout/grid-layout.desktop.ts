import { GridLayoutBase, ItemSpec as ItemSpecBase, layout } from "@nativescript/core/ui/layouts/grid-layout/grid-layout-common";
...
function createNativeSpec(itemSpec: ItemSpecBase): any {
...
export class ItemSpec extends ItemSpecBase {
  ...
  public get actualLength(): number {
    if (this.nativeSpec) {
      return Math.round(
        this.nativeSpec.getActualLength() / layout.getDisplayDensity()
      );
    }
...
export class GridLayout extends GridLayoutBase {
  ...
  public createNativeView() {
    ...
    return view; // new org.nativescript.widgets.GridLayout(this._context);
  }

  public initNativeView(): void {
    super.initNativeView();
    // Update native GridLayout
    super.rowsInternal.forEach((itemSpec: ItemSpec, index, rows) => {
      super._onRowAdded(itemSpec);
    }, this);
    super.columnsInternal.forEach((itemSpec: ItemSpec, index, rows) => {
      super._onColumnAdded(itemSpec);
    }, this);
  }

  public resetNativeView() {
    // Update native GridLayout
    for (let i = super.rowsInternal.length; i--; i >= 0) {
      const itemSpec = <ItemSpec>super.rowsInternal[i];
      super._onRowRemoved(itemSpec, i);
    }

    for (let i = super.columnsInternal.length; i--; i >= 0) {
      const itemSpec = <ItemSpec>super.columnsInternal[i];
      super._onColumnRemoved(itemSpec, i);
    }
