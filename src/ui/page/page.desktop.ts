...
import { actionBarHiddenProperty, PageBase, statusBarStyleProperty, ViewBase } from "@nativescript/core/ui/page/page-common";
// @ts-ignore
...
export class Page extends PageBase {
  ...
  public initNativeView(): void {
    ...
    (<View>(<unknown>this)).styles.set("flex-direction", "column").apply();
  }
  ...
  public _addViewToNativeVisualTree(view: View, atIndex?: number): boolean {
    if (this.nativeViewProtected && view.nativeViewProtected) {
      if (view instanceof ActionBar) {
        ...
        (<View>view).styles.set("flex", "1");
      }

      (<View>view).styles.apply();
    }

    this.nativeViewProtected.removeEventListener(
  ...
  _resizeHandler() {
    super.eachChild((view): boolean => {
      ...
      this._actionBarWidget.setInlineStyle(
        `max-height: 60; width: ${size.width()}`
      );
      this._contentWidget.setInlineStyle(
        `width: ${size.width()}; height: ${size.height() - 60}`
      );
  ...
  [statusBarStyleProperty.setNative](
