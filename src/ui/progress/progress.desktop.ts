import { backgroundInternalProperty, Color, CssProperty, InheritedCssProperty, makeParser, makeValidator, maxValueProperty, ProgressBase, Style, valueProperty } from "@nativescript/core/ui/progress/progress-common";
...
export class Progress extends ProgressBase {
  ...
  styles: StyleList = new StyleList(<View>(<unknown>this), "QProgressBar", [
    "chunk"
  ]);
  ...
  [desktopChunkColorProperty.setNative](value: Color) {
    this.styles
      .set(
        "background-color",
        value instanceof Color ? value.desktop : value,
        "chunk"
      )
      .apply();
  }
  ...
  [desktopChunkMarginProperty.setNative](value: number) {
    this.styles.set("margin", value, "chunk").apply();
  }

  [desktopChunkWidthProperty.setNative](value: Color) {
    this.styles.set("width", value, "chunk").apply();
  }

  [textAlignmentProperty.setNative](value: string) {
    this.styles.set("text-align", value).apply();
  }
