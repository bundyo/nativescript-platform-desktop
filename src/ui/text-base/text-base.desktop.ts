...
import { TextAlignment, TextDecoration, TextTransform, WhiteSpace } from "@nativescript/core/ui/text-base/text-base";
import { Color, colorProperty, fontInternalProperty, FormattedString, formattedTextProperty, letterSpacingProperty, lineHeightProperty, resetSymbol, Span, textAlignmentProperty, TextBaseCommon, textDecorationProperty, textProperty, textTransformProperty, whiteSpaceProperty } from "@nativescript/core/ui/text-base/text-base-common";
...
const majorVersion = desktop.MajorVersion;

export class TextBase extends TextBaseCommon {
  public nativeViewProtected: QPushButton | QLabel;
  public nativeTextViewProtected: QPushButton | QLabel;
  ...
  [colorProperty.setNative](value: any) {
    const color = value instanceof Color ? value.desktop : value;
    const nativeView = this.nativeTextViewProtected;
    // if (nativeView instanceof UIButton) {
    //     nativeView.setTitleColorForState(color, UIControlState.Normal);
    //     nativeView.titleLabel.textColor = color;
    // } else {
    nativeView.setInlineStyle(`color: ${color}`);
    // }
  }
  ...
  [fontInternalProperty.setNative](value: any) {
    if (!(value instanceof Font) || !super.formattedText) {
      let nativeView = this.nativeTextViewProtected;
      // nativeView = nativeView instanceof UIButton ? nativeView.titleLabel : nativeView;
      // const font = value instanceof Font ? value.getUIFont(nativeView.font) : value;
      //nativeView.font = value; // font;
    }
  }
  ...
  _setNativeText(reset: boolean = false): void {
    ...
    if ((<TextBaseCommon>this).formattedText) {
  ...
  setFormattedTextDecorationAndTransform() {
    ...
    if ((<TextBaseCommon>this).letterSpacing !== 0) {
    ...
    this.nativeTextViewProtected.setText(
      (<TextBaseCommon>this).formattedText.toString()
    );
  }

  setTextDecorationAndTransform() {
    const style = (<TextBaseCommon>this).style;
    ...
    // }
    //
    const text = (<TextBaseCommon>this).text;
    ...
    //     }
    // }
    this.nativeTextViewProtected.setText(string);
  }
