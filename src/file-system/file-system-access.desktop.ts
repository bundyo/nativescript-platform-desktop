...
export class FileSystemAccess {
  ...
  public getFile(
    path: string,
    onError?: (error: any) => any
  ): { path: string; name: string; extension: string } | any {
    // try {
    //     const fileManager = NSFileManager.defaultManager;
  ...
  public eachEntity(
    path: string,
    onEntity: (file: { path: string; name: string; extension: string }) => any,
    onError?: (error: any) => any
  ) {
    if (!onEntity) {
      return;
  ...
  private exists(
    path: string
  ): { exists: boolean; isDirectory: boolean } | any {
    // const fileManager = NSFileManager.defaultManager;
