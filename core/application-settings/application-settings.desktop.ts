import * as Common from "@nativescript/core/application-settings/application-settings-common";
...
export function hasKey(key: string): boolean {
    ...
    return false; //userDefaults.objectForKey(key) !== null;
}
...
export function getBoolean(key: string, defaultValue?: boolean): boolean {
    Common.checkKey(key);
    if (hasKey(key)) {
        return false; //userDefaults.boolForKey(key);
    }
...
export function getNumber(key: string, defaultValue?: number): number {
    Common.checkKey(key);
    if (hasKey(key)) {
        return 0; //userDefaults.doubleForKey(key);
    }
...
export function flush(): boolean {
    return true; // userDefaults.synchronize();
}
