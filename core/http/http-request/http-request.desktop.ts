...
import * as domainDebugger from "@nativescript/core/debugger/debugger";
...
const currentDevice = { systemVersion: "10.0" };
const device = "Desktop";
const osVersion = currentDevice.systemVersion;

const GET = "GET";
const USER_AGENT_HEADER = "User-Agent";
const USER_AGENT = `Mozilla/5.0 (${device}; CPU OS ${osVersion.replace(".", "_")} like Mac OS X) AppleWebKit/536.26 (KHTML, like Gecko) Version/${osVersion} Mobile/10A5355d Safari/8536.25`;
...
export function request(options: httpModule.HttpRequestOptions): Promise<httpModule.HttpResponse> {
    return new Promise<httpModule.HttpResponse>((resolve, reject) => {
        ...
        //     dataTask.resume();
            ...
            // @ts-ignore
            resolve({ content: { raw: 'Not implemented!' } });
        } catch (ex) {
            reject(ex);
        }
    });
