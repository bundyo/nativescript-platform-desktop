// Definitions.
import {
    BackstackEntry, NavigationTransition, traceError
...
} from "@nativescript/core/ui/frame/frame-common";
...
export class Frame extends FrameBase {
    ...
    public _navigateCore(backstackEntry: BackstackEntry) {
        ...
        const isReplace = (<FrameBase>this)._executingContext && (<FrameBase>this)._executingContext.navigationType === NavigationType.replace;
        if (!isReplace) {
            navDepth++;
        }

        let navigationTransition: NavigationTransition;
        let animated = (<FrameBase>this).currentPage ? (<FrameBase>this)._getIsAnimatedNavigation(backstackEntry.entry) : false;
        if (isReplace) {
            animated = true;
            navigationTransition = { name: HMR_REPLACE_TRANSITION, duration: 100 };
            // viewController[TRANSITION] = navigationTransition;
        } else if (animated) {
            navigationTransition = (<FrameBase>this)._getNavigationTransition(backstackEntry.entry);
...
//        if (!this._currentEntry) {
            ...
            this.setCurrent(backstackEntry, (<FrameBase>this)._executingContext.navigationType);
...
//        }
    ...
    public _goBackCore(backstackEntry: BackstackEntry) {
        ...
        let animated = (<FrameBase>this)._currentEntry ? (<FrameBase>this)._getIsAnimatedNavigation((<FrameBase>this)._currentEntry.entry) : false;
